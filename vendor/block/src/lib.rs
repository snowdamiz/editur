...
pub trait BlockArguments: Sized {
    ...
    /// safety requirements of the invoked foreign code.
    ...
    A: BlockArguments,
{
    ...
    /// # Safety
    ///
    /// This invokes foreign code that the caller must verify
...
impl<A, R> RcBlock<A, R> {
    ...
    /// # Safety
    ///
    /// `ptr` must point to a valid `Block` and have a +1 reference count or it
    ...
    ($f:ident, $($a:ident : $t:ident),*) => (
        impl<$($t,)* R, X> IntoConcreteBlock<($($t,)*)> for X
            ...
            fn into_concrete_block(self) -> ConcreteBlock<($($t,)*), R, X> {
                ...
                unsafe {
                    ConcreteBlock::with_invoke(mem::transmute::<
                        unsafe extern "C" fn(
                            *mut ConcreteBlock<($($t,)*), R, X>
                            $(, $a: $t)*
                        ) -> R,
                        unsafe extern "C" fn(
                            *mut ConcreteBlock<($($t,)*), R, X>, ...
    ...
    fn clone(&self) -> Self {
        unsafe {
            ConcreteBlock::with_invoke(
                mem::transmute::<
                    unsafe extern "C" fn(*mut Block<A, R>, ...) -> R,
                    unsafe extern "C" fn(*mut ConcreteBlock<A, R, F>, ...) -> R,
                >(self.base.invoke),
                self.closure.clone(),
            )
        }
