...
use futures::{AsyncBufRead, AsyncRead, AsyncWrite, Stream, StreamExt};
...
// Fits a 16 MiB binary payload after base64 expansion, plus JSON metadata.
...
pub(crate) fn bounded_lines<R>(reader: R) -> impl Stream<Item = std::io::Result<String>>
where
    R: AsyncBufRead,
{
    BoundedLines {
...
impl<R> Stream for BoundedLines<R>
where
    R: AsyncBufRead,
{
    ...
    fn poll_next(
        ...
        loop {
            let available = match std::task::ready!(this.reader.as_mut().poll_fill_buf(cx)) {
            ...
            if available.is_empty() {
                ...
                return std::task::Poll::Ready(Some(decode_line(std::mem::take(&mut this.bytes))));
            }

            let newline = available.iter().position(|byte| *byte == b'\n');
            let take = newline.unwrap_or(available.len());
            if this.bytes.len().saturating_add(take) > MAX_FRAME_BYTES {
                this.done = true;
                return std::task::Poll::Ready(Some(Err(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!("ACP frame exceeds {MAX_FRAME_BYTES} bytes"),
                ))));
            }
            this.bytes.extend_from_slice(&available[..take]);
            this.reader
                .as_mut()
                .consume(take + usize::from(newline.is_some()));

            if newline.is_some() {
                let mut bytes = std::mem::take(&mut this.bytes);
                if bytes.last() == Some(&b'\r') {
                    bytes.pop();
                }
                return std::task::Poll::Ready(Some(decode_line(bytes)));
            }
        }
    }
}

fn decode_line(bytes: Vec<u8>) -> std::io::Result<String> {
    ...
    IB: AsyncRead + Send + 'static,
{
    ...
    > {
        ...
        let incoming_lines = Box::pin(bounded_lines(BufReader::new(incoming)));
        let outgoing_lines =
