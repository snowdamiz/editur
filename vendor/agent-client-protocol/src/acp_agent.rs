...
impl AcpAgent {
    ...
    /// Stop the launched agent when the supplied flag becomes true.
    #[must_use]
...
impl<Counterpart: AcpAgentCounterpartRole> crate::ConnectTo<Counterpart> for AcpAgent {
    async fn connect_to(
        ...
        > = if let Some(callback) = self.debug_callback.clone() {
            Box::pin(
                crate::jsonrpc::bounded_lines(BufReader::new(child_stdout)).inspect(
                    move |result| {
            ...
            Box::pin(crate::jsonrpc::bounded_lines(BufReader::new(child_stdout)))
        };
        ...
        let main_race = Box::pin(main_race);
        ...
        let main_race = async move {
            match futures::future::select(main_race, cancelled).await {
                futures::future::Either::Left((result, _)) => result,
                futures::future::Either::Right(((), main_race)) => {
                    drop(main_race);
                    Ok(())
                }
            }
